...
class ImplicitHyperlinkTargetPassTest extends TestCase
{
    public function testAllImplicitUniqueSections(): void
    {
        ...
        foreach (['Document 1', 'Section A', 'Section B'] as $titles) {
            $document->addChildNode(new SectionNode(new TitleNode(new SpanNode($titles), 1, $slugger->slug($titles)->lower()->toString())));
            $expected->addChildNode(new SectionNode(new TitleNode(new SpanNode($titles), 1, $slugger->slug($titles)->lower()->toString())));
        }
    ...
    public function testExplicit(): void
    {
        ...
        $document->addChildNode(new SectionNode(new TitleNode(new SpanNode('Document 1'), 1, 'document-1')));
        $expected->addChildNode(new SectionNode(new TitleNode(new SpanNode('Document 1'), 1, 'document-1')));
        ...
        $document->addChildNode(new SectionNode(new TitleNode(new SpanNode('Section A'), 1, 'section-a')));
